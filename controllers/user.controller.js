...
export const addRequestTeamForm = async (req, res, next) => {
  try {
    const newRequestTeamForm = new RequestTeamForm(req.body);

    await newRequestTeamForm.save();
    res.status(200).json(newRequestTeamForm);
  } catch (error) {
    next(error);
  }
};
